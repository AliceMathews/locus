...
export default function SavedSuccess(props) {
  ...
  const photoInfo = reqInfo.map((setting, i) => {
    let key = setting[0];
    let value = (setting[1] === null)? "" : setting[1];
    
    if (setting[0] === 'aperture') {
      value = (setting[1] === null)? "" : 'F' + setting[1];
    } else if (setting[0] === 'shutter_speed') {
      key = 'shutter speed';
      value = (setting[1] === null)? "" : setting[1] + ' ms';
    } else if (setting[0] === 'exposure') {
      value = (setting[1] === null)? "" : ((setting[1] >= 0)? `+ ${setting[1]}` : setting[1]);
    } else if (setting[0] === 'focul_length') {
      key = 'focal length';
      value = (setting[1] === null)? "" : `${setting[1]} mm`;
    }
